<?php
/**
 * Zhazhasu炸炸酥网安靱场团队 - 密码重置会话覆盖靶场 - 第二关
 * 版本: v1.0.0
 * 创建日期: 2026-01-23
 * 团队: 炸炸酥网安靱场 (Zhazhasu)
 *
...
// 设置响应头
header('X-Zhazhasu: Zhazhasu 密码重置会话覆盖 Range v1.0.0');
...
{
    $stmt = $pdo->prepare("SELECT * FROM zhazhasu_sessionoverride_users WHERE level = ? AND username = 'admin'");
    $stmt->execute([$level]);
    ...
    $stmt = $pdo->prepare("INSERT INTO zhazhasu_sessionoverride_users (level, username, password, user_id, phone, is_admin) VALUES (?, 'admin', ?, ?, ?, 1)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    $stmt = $pdo->prepare("SELECT * FROM zhazhasu_sessionoverride_users WHERE level = ? AND username = ?");
