<?php
/**
 * Zhazhasu炸炸酥网安靱场团队 - 密码重置凭证可猜测靶场 - 第二关密码重置页面
 * 版本: v1.0.0
 * 创建日期: 2026-01-22
 * 团队: 炸炸酥网安靱场 (Zhazhasu)
 *
...
if ($token) {
    ...
    //验证token是否等于用户名的MD5值
    $stmt = $pdo->prepare("SELECT * FROM zhazhasu_resetlink_users WHERE level = 2 AND MD5(username) = ?");
    $stmt->execute([$token]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'reset_password' && $valid) {
    ...
    if (empty($newPassword)) {
        ...
        $pdo = Zhazhasu_Database::getConnection('zhazhasu_logic');
        $stmt = $pdo->prepare("UPDATE zhazhasu_resetlink_users SET password = ? WHERE level = 2 AND username = ?");
