<?php
/**
 * Zhazhasu炸炸酥网安靱场团队 - 密码重置凭证可猜测靶场 - 添加好友API（第三关）
 * 版本: v1.0.0
 * 创建日期: 2026-01-22
 * 团队: 炸炸酥网安靱场 (Zhazhasu)
 */
...
try {
    ...
    if ($action === 'search') {
        // 查找好友账号
        $stmt = $pdo->prepare("SELECT username, user_id, phone FROM zhazhasu_resetlink_users WHERE level = 3 AND username = ?");
        ...
        // 检查是否已添加好友
        $stmt = $pdo->prepare("SELECT friend_added FROM zhazhasu_resetlink_users WHERE level = 3 AND id = ?");
    ...
    if ($action === 'add') {
        ...
        // 更新用户记录，标记已添加好友
        $stmt = $pdo->prepare("UPDATE zhazhasu_resetlink_users SET friend_added = 1, friend_username = ?, updated_at = CURRENT_TIMESTAMP WHERE level = 3 AND id = ?");
