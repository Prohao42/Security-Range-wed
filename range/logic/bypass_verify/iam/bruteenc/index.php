...
// 设置响应头
header('X-Zhazhasu: Zhazhasu BruteEnc Range v1.0.0 - Level 1');
...
{
    // 检查admin用户是否存在（第一关）
    $stmt = $pdo->prepare("SELECT username, password FROM zhazhasu_bruteenc_users WHERE username = 'admin' AND level = 1 LIMIT 1");
    $stmt->execute();
    ...
    $stmt = $pdo->prepare("INSERT INTO zhazhasu_bruteenc_users (username, password, level, created_at) VALUES (?, ?, 1, NOW())");
