...
// 设置响应头
header('X-Zhazhasu: Zhazhasu UserEnum Range v1.0.0 - Level 1');
...
{
    // 检查测试账号是否存在
    $stmt = $pdo->prepare("SELECT id FROM zhazhasu_userenum_users WHERE username = '13866668888' AND level = 1 LIMIT 1");
    $stmt->execute();
    if (!$stmt->fetch()) {
        // 创建测试账号
        $stmt = $pdo->prepare("INSERT INTO zhazhasu_userenum_users (username, password, level, created_at) VALUES (?, ?, 1, NOW())");
        $stmt->execute(['13866668888', '123456']);
    }

    // 检查目标账号是否存在（1100591XXXX）
    $stmt = $pdo->prepare("SELECT username FROM zhazhasu_userenum_users WHERE username LIKE '1100591%' AND level = 1 LIMIT 1");
    $stmt->execute();
    if (!$stmt->fetch()) {
        // 生成目标账号
