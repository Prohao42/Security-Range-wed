...
class Zhazhasu_SessionManager {
    ...
    private static function loadProgress() {
        try {
            $stmt = self::$db->prepare(
                "SELECT * FROM zhazhasu_xss_content_filter_progress WHERE session_id = ? LIMIT 1"
            );
    ...
    private static function createProgress() {
        try {
            $stmt = self::$db->prepare(
                "INSERT INTO zhazhasu_xss_content_filter_progress
    ...
    public static function completeLevel($level) {
        ...
        try {
            ...
            // 更新数据库
            $stmt = self::$db->prepare(
                "UPDATE zhazhasu_xss_content_filter_progress
    ...
    private static function recordAchievement($level) {
        try {
            ...
            // 插入或更新成就记录
            $stmt = self::$db->prepare(
                "INSERT INTO zhazhasu_xss_content_filter_records
    ...
    public static function getStarCount() {
        ...
        try {
            $stmt = self::$db->query(
                "SELECT COUNT(*) as count FROM zhazhasu_xss_content_filter_records"
            );
    ...
    public static function resetProgress() {
        ...
        try {
            // 重置进度表
            $stmt = self::$db->prepare(
                "DELETE FROM zhazhasu_xss_content_filter_progress WHERE session_id = ?"
            );
            $stmt->execute([self::$sessionId]);

            // 重置成就表
            $stmt = self::$db->prepare(
                "DELETE FROM zhazhasu_xss_content_filter_records"
            );
            $stmt->execute();
