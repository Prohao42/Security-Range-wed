...
-- 删除已存在的表（确保重置到初始状态）
DROP TABLE IF EXISTS `zhazhasu_xss_content_filter_records`;
DROP TABLE IF EXISTS `zhazhasu_xss_content_filter_progress`;

-- 创建成就记录表
CREATE TABLE IF NOT EXISTS `zhazhasu_xss_content_filter_records` (
...
-- 创建关卡进度表
CREATE TABLE IF NOT EXISTS `zhazhasu_xss_content_filter_progress` (
    `id` int(11) NOT NULL AUTO_INCREMENT,
